#!/usr/bin/env python3
"""
Create Draft Email for Specific Order
Creates a draft email for a specific order ID from raw.csv
"""
...
def get_order_by_id(order_id):
    """Get specific order from raw.csv by order ID."""
    try:
        ...
        for order in all_orders:
            if order.get('Order ID', '').strip() == order_id.strip():
                return order
...
def create_draft_for_order(order_id):
    """Create a test draft email for specific order."""
    try:
        print("="*80)
        print(f"CREATING DRAFT EMAIL FOR ORDER: {order_id}")
        print("="*80)
        
        # Get order
        print(f"\n[INFO] Loading order {order_id} from raw.csv...")
        order = get_order_by_id(order_id)
        
        if not order:
            print(f"[ERROR] Order {order_id} not found in raw.csv")
            return
        ...
        print(f"  Scheduled Delivery: {scheduled_delivery}")
...
if __name__ == "__main__":
    # Default order ID
    order_id = "TT251111001"
    
    # Check if order ID provided as command line argument
    if len(sys.argv) > 1:
        order_id = sys.argv[1]
    
    create_draft_for_order(order_id)
